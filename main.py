...
class MainApp(App):
    ...
    def on_start(self):
        ...
        # carregar infos usuario
        self.carregar_infos_usuaruio()

    def carregar_infos_usuaruio(self):
        try:
            ...
            # pegar informacoes usuario
            requisicao = requests.get(f"https://aplicativovendasjoao-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}")
    ...
    def mudar_foto_perfil(self, foto, *args):
        ...
        info = f'{{"avatar": "{foto}"}}'
        requisicao = requests.patch(f'https://aplicativovendasjoao-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}',
                                    data=str(info))

        self.mudar_tela("ajustespage")

    def adicionar_vendedor(self, id_vendedor_adicionado):
        link = f'https://aplicativovendasjoao-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"&equalTo="{id_vendedor_adicionado}"'
        requisicao = requests.get(link)
        requisicao_dic = requisicao.json()
        print((requisicao_dic))

        pagina_adicionarvendedor = self.root.ids["adicionarvendedorpage"]
        mensagem_texto = pagina_adicionarvendedor.ids["mensagen_outrovendedor"]
        ...
        # if requesicao_dic == {}:
        #     mensagen_texto.text = "Usuário não encontrado"
        # else:
        #     equipe = self.equipe.split(" ,")
        #     if id_vendedor_adicionado in equipe:
        #         mensagen_texto.text = "Vendedor já faz parte da equipe"
        #     else:
        #         self.equipe = self.equipe + f",{id_vendedor_adicionado}"
        #         info = f'{{"equipe": "{self.equipe}"}}'
        #         requests.patch(f"https://aplicativovendasjoao-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}",
        #                        data=info)
        #
        #         mensagen_texto.text = "Vendedor Adicionado com Sucesso"
        #         # adicionar um novo banner na lista de vendedores
        #         pagina_listavendedores = self.root.ids["listarvendedorespage"]
        #         lista_vendedores = pagina_listavendedores.ids["lista_vendedores"]
        #         banner_vendedor = BannerVendedor(id_vendedor=id_vendedor_adicionado)
    ...
    def selecionar_unidades(self, id_label, *args):
        ...
        # pintar o cara selecionar de azul
    ...
    def adicionar_venda(self):
        ...
        pagina_adicionarvendas = self.root.ids["adicionarvendaspage"]
        data = pagina_adicionarvendas.ids["label_data"].text.replace("Date: ", "")
        ...
        if cliente and produto and unidade and preco and quantidade and (type(preco) == float) and (type(quantidade) == float):
            ...
            info = f'{{"cliente": "{cliente}", "quantidade": "{quantidade}", "produto": "{produto}",' \
                   f' "foto_cliente": "{foto_cliente}","foto_produto":"{foto_produto}", "data": "{data}",' \
                   f' "unidade": "{unidade}", "preco": "{preco}"}}'


            requests.post(f'https://aplicativovendasjoao-default-rtdb.firebaseio.com/{self.local_id}/vendas.json?auth={self.id_token}',
                          data=info)
            ...
            requisicao = requests.get(f'https://aplicativovendasjoao-default-rtdb.firebaseio.com/{self.local_id}/total_vendas.json?auth={self.id_token}')
            total_vendas = float(requisicao.json())
            total_vendas += preco
            info = f'{{"total_vendas": "{total_vendas}"}}'
            requests.patch(f"https://aplicativovendasjoao-default-rtdb.firebaseio.com/{self.local_id}.json?auth={self.id_token}", data=info)
            homepage = self.root.ids["homepage"]
            homepage.ids["label_total_vendas"].text = f'[color=#000000]Total de Vendas:[/color] [b]R${total_vendas}[/b]'
            self.mudar_tela("homepage")
        self.cliente = None
    ...
    def carregar_todas_vendas(self):
        ...
        # preencher a pagina todas vendas page
        # pegar informacoes empresa
        requisicao = requests.get (f'https://aplicativovendasjoao-default-rtdb.firebaseio.com/.json?orderBy="id_vendedor"')
        requisicao_dic = requisicao.json()
        print(requisicao_dic)
